...
export default function InspeccionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const inspeccionId = resolvedParams.id
  ...
  // Load inspección directamente por su ID
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Cargar la inspección directamente por su ID
        const detalle = await getInspeccionDetalle(parseInt(inspeccionId), token)
        ...
        setLoadError(error.message || "Error al cargar la inspección")
  ...
  // Si hubo un error al cargar o no hay inspección, mostrar error
  if (loadError || !inspeccionExistente) {
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Inspección no encontrada</h1>
            <p className="text-muted-foreground mt-1">ID: {inspeccionId}</p>
          </div>
        </div>
        ...
        <Card className="border-red-200 bg-red-50/50 dark:border-red-900/50 dark:bg-red-950/20">
          <CardHeader>
            ...
            <CardDescription className="text-red-600 dark:text-red-400">
              {loadError || "No se encontró la inspección solicitada."}
            </CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <Button variant="ghost" size="sm" asChild className="sm:size-icon sm:p-2">
            <Link href="/dashboard/inspecciones" className="flex items-center gap-1">
              <ArrowLeft className="h-5 w-5" />
          ...
          <div className="hidden sm:block">
            <h1 className="text-3xl font-bold tracking-tight">
              {inspeccionExistente.numero_inspeccion}
            </h1>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="icon" asChild>
                  <Link href={`/dashboard/ot/${inspeccionExistente.orden_trabajo}`}>
